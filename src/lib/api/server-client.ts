...
// their assigned deanery/parish/outstation.
export function applyCallerScope(
  requested: { deaneryId?: string | null; parishId?: string | null; outstationId?: string | null },
  caller: {
    scopeDeaneryId: string | null;
    scopeParishId: string | null;
    scopeOutstationId: string | null;
  },
): { deaneryId: string | null; parishId: string | null; outstationId: string | null } {
  return {
    deaneryId: caller.scopeDeaneryId ?? requested.deaneryId ?? null,
    parishId: caller.scopeParishId ?? requested.parishId ?? null,
    outstationId: caller.scopeOutstationId ?? requested.outstationId ?? null,
  };
}
...
> {
  ...
  const scopeOutstationId = profileData?.outstation_id ?? null;

  return { userId: auth.userId, role, scopeDeaneryId, scopeParishId, scopeOutstationId };
}
