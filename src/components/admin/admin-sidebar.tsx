...
function Badge({ text, tone }: { text: string; tone: "danger" | "gold" | "info" }) {
  const styles =
    tone === "danger"
  ...
  return (
    <aside className="flex h-screen w-[220px] shrink-0 flex-col border-r border-border bg-card">
      ...
      <div className="flex items-center gap-2.5 border-b border-border px-3.5 py-4">
        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-gold text-[11px] font-black text-gold-foreground">
          CDM
        </div>
            ...
            {group.items.map((item) => {
              ...
              return (
                <Link
                  key={item.to}
                  to={item.to}
                  className={`relative mb-[2px] flex items-center gap-2.5 rounded-lg px-2.5 py-[7px] text-[12px] font-medium transition-colors ${
                    isActive
