...
import { Topbar, TopbarTab, TopbarButton } from "@/components/admin/layout/topbar";
import {
  Card,
  CardBody,
  CardHead,
...
import { ANALYTICS_UNITS, ORGANIZATION, TOP_PARISHES, type AnalyticsUnit } from "@/lib/mock-data";
...
function useRpcFilters() {
  ...
  const effectiveOutstationId = adminScope.outstationId || filters.outstationId || null;
  ...
  const groupBy: GroupBy = effectiveParishId
    ? "outstation"
    : effectiveDeaneryId
      ? "parish"
      : "deanery";
...
function ChartDisplayToggle({
  ...
  value: ChartDisplay;
  onChange: (value: ChartDisplay) => void;
}) {
  return (
...
function genderRows(units: AnalyticsUnit[], metric: "enrolled" | "missionNominees") {
  ...
  const female = units.reduce(
    (sum, unit) => sum + Math.round(unit[metric] * (0.46 + (unit.youths % 13) / 100)),
    0,
  );
  return [
...
function GeneralTab({ chartDisplay }: { chartDisplay: ChartDisplay }) {
  ...
  const catTotals = youthBreakdown.reduce(
    (acc, r) => ({
      primary: acc.primary + r.cat_primary,
      secondary: acc.secondary + r.cat_secondary,
      tertiary: acc.tertiary + r.cat_tertiary,
      working: acc.working + r.cat_working,
    }),
  ...
  return (
    <>
      ...
      <div className="mb-3.5 grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-5">
        <Kpi
          label="Total Youths"
          value={summaryLoading ? "—" : totalYouths.toLocaleString()}
          trend="selected scope"
          tone="info"
          sub={rpc.scopeLabel}
        />
        <Kpi
          label="Enrolled"
          value={summaryLoading ? "—" : totalEnrolled.toLocaleString()}
          trend={summaryLoading ? "loading…" : `${pct(totalEnrolled, totalYouths)}% of registered`}
          tone="up"
        />
        <Kpi
          label="CUSA Members"
          value={summaryLoading ? "—" : (summary?.cusa_members ?? 0).toLocaleString()}
          trend={summaryLoading ? "loading…" : `${summary?.cusa_active ?? 0} active`}
          tone="up"
        />
        <Kpi
          label="Active Leaders"
          value={summaryLoading ? "—" : (summary?.active_leaders ?? 0).toLocaleString()}
          trend="in current term"
          tone="info"
        />
        <Kpi
          label="Upcoming Events"
          value={summaryLoading ? "—" : upcomingCount.toLocaleString()}
          trend="next 30 days"
          tone="info"
          sub="diocese-wide"
        />
                ...
                data={
                  ageRangeData.length > 0
                    ? ageRangeData.map((r, i) => ({
                        label: r.label,
                        value: r.count,
                        color: ["var(--color-info)", "var(--color-success)", "var(--color-violet)"][
                          i % 3
                        ],
                      }))
                    : [{ label: "No data", value: 1, color: "var(--color-border)" }]
            ...
            <CardBody className="space-y-2">
              {upcomingCount > 0 && (
                <div className="rounded-lg border border-border bg-bg-3 px-3 py-2 text-center text-[10px] text-text-1">
                  ...
                  <a href="/admin/events" className="text-danger hover:underline">
                    view all →
                  </a>
                </div>
              )}
  ...
  return (
    <FilterBar>
      ...
      <select
        ...
        onChange={(event) =>
          setFilters({ deaneryCode: event.target.value, parishId: "", churchId: "" })
        }
...
const SEL_CLS =
  "min-w-[148px] rounded-md border border-black/20 bg-white px-2.5 py-1.5 text-[11px] font-semibold text-black/70 outline-none transition-colors disabled:opacity-40 hover:border-gold-3/50 hover:text-black focus:border-gold-3 focus:text-black";
  ...
  return (
    <FilterBar>
      ...
      <select
        ...
        onChange={(e) =>
          rpc.setFilters({ deaneryId: e.target.value, parishId: "", outstationId: "" })
        }
        className={SEL_CLS}
      >
        <option value="">All Deaneries</option>
        {rpc.deaneryOptions.map((o) => (
      ...
      <select
        ...
        onChange={(e) =>
          rpc.setFilters((f) => ({ ...f, parishId: e.target.value, outstationId: "" }))
        }
        className={SEL_CLS}
      >
        <option value="">All Parishes</option>
        {rpc.parishOptions.map((o) => (
          <option key={o.value} value={o.value}>
            {o.label}
          </option>
        ))}
      ...
      <select
        ...
        disabled={!!rpc.adminScope.outstationId || !rpc.effectiveParishId}
        onChange={(e) => rpc.setFilters((f) => ({ ...f, outstationId: e.target.value }))}
        className={SEL_CLS}
      >
        <option value="">All Outstations</option>
        {rpc.outstationOptions.map((o) => (
          <option key={o.value} value={o.value}>
            {o.label}
          </option>
        ))}
    ...
    <>
      ...
      <div className="mb-3.5 flex flex-wrap items-start gap-3.5 rounded-xl border border-danger/20 bg-gradient-to-r from-danger-soft via-white to-warn-soft p-4">
        ...
        <div className="min-w-[180px] flex-1">
          <div className="text-[13px] font-bold text-foreground">
            Enrollment Window — Year {year}
          </div>
          <div className="text-[10px] text-text-3">
            Showing data for {year} · {totalEnrolled.toLocaleString()} enrolled of{" "}
            {totalYouths.toLocaleString()} registered
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Stat
            value={Math.max(0, totalYouths - totalEnrolled).toLocaleString()}
            label="remaining"
            tone="warn"
          />
        </div>
      ...
      <div className="mb-3.5 grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-5">
        <Kpi
          ...
          trend={`${completionPct}% of registered`}
          tone="up"
          sub={`of ${totalYouths.toLocaleString()}`}
        />
        <Kpi
        ...
        <Kpi
          label="Waived"
          value={totalWaived.toLocaleString()}
          trend="fee waived"
          tone="info"
          sub="this year"
        />
          ...
          <CardBody className="h-[240px]">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={trendRows} margin={{ top: 10, right: 10, left: -16, bottom: 0 }}>
                ...
                <Tooltip
                  formatter={(v) =>
                    chartDisplay === "percent" ? `${v}%` : Number(v).toLocaleString()
                  }
                  contentStyle={{
    ...
    <>
      ...
      <div className="mb-3.5 grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        <Kpi
          ...
          trend={`${totalActive} active`}
          tone="up"
          accent="var(--color-violet)"
          sub={institution || rpc.scopeLabel}
        />
        <Kpi
          label="Active Members"
          value={totalActive.toLocaleString()}
          trend={`${activityPct}% activity`}
          tone="up"
          accent="var(--color-violet)"
        />
        <Kpi
          label="Chapters"
          value={cusaBreakdown.length.toLocaleString()}
          trend="reporting"
          tone="info"
          accent="var(--color-violet)"
        />
          ...
          <CardBody>
            <Donut
              data={[
                ...
                {
                  label: "No Role",
                  value: Math.max(0, totalMembers - totalWithRole),
...
function LeadersTab({ chartDisplay }: { chartDisplay: ChartDisplay }) {
  ...
  const orgColLabel =
    groupBy === "parish" ? "Parish" : groupBy === "outstation" ? "Outstation" : "Deanery";
      ...
      <div className="mt-3">
        <Card>
          <CardHead
            ...
            subtitle={`Active leaders by ${orgColLabel.toLowerCase()} and role level`}
          />
              ...
              <TableHeader>
                <TableRow>
                  {[orgColLabel, "Diocese", "Deanery", "Parish", "Outstation", "Total"].map((h) => (
                    <TableHead key={h} className="label-eyebrow px-3 py-2">
                      {h}
                    </TableHead>
                  ))}
                </TableRow>
              ...
              <TableBody>
                {breakdown.map((row) => (
                  <TableRow key={row.id ?? row.label} className="border-border/30 hover:bg-bg-3">
                    <TableCell className="px-3 py-2 text-[10px] font-semibold text-foreground">
                      {row.label}
                    </TableCell>
                    ...
                    <TableCell className="px-3 py-2 text-[10px] font-bold text-foreground">
                      {row.total_active}
                    </TableCell>
                  </TableRow>
                ))}
  ...
  return (
    <>
      ...
      <div className="mb-3.5 grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-4">
        <Kpi
          label="Mission Nominees"
          value="0"
          trend="no mission table"
          tone="up"
          sub={rpc.scopeLabel}
        />
        <Kpi
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1.25fr_1fr]">
        <Card>
          <CardHead
            title="Mission Nominees"
            subtitle="No mission table is provisioned yet, so counts remain zero."
          />
          <CardBody>
